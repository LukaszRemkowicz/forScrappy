...
async def test_link_validator() -> None:
    ...
    validator: LinkValidator = LinkValidator(link=f"{settings.local.base_url}trance/")
...
def test_validate_category_in_link() -> None:
    ...
    assert validate_category(link=f"{settings.local.base_url}trance/") == "trance"
...
def test_validate_category_not_in_link() -> None:
    ...
    with pytest.raises(ValueError):
        validate_category(link=f"{settings.local.base_url}ttt/")

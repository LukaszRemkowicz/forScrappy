...
from typing import List, Optional, Pattern, Dict, TypeVar
...
class BaseParser:
    ...
    async def parse_date(self, tag_element: List[Tag]) -> Optional[Dict]:
        raise NotImplementedError
...
class KrakenParser(BaseParser):
    ...
    async def parse_date(self, tag_elements: List[Tag]) -> Optional[datetime]:
        ...
        for li_tag in tag_elements:
            ...
            if element_type.text == "Upload date":
                date = li_tag.find("div", {"class": "lead-text"}).text
    ...
    async def parse_name(self, soup: BeautifulSoup) -> Optional[str]:
        ...
        name = soup.find("div", {"class": regex}).find("span", {"class": "coin-name"}).text
        minified = await self.printify_name(name)
        return minified
    ...
    async def get_download_link(self, page_link: str) -> Optional[Dict]:
        ...
        # parse token
        token: str = soup.find("input", id="dl-token")["value"]
        ...
        payload: str = f'------WebKitFormBoundary7MA4YWxkTrZu0gW\r\nContent-Disposition: form-data; name="token"' \
        ...
        dl_link_resp: Response = self.session.post(
            f"{settings.kraken_base_url}/download/{dl_hash}", data=payload, headers=headers
        )

        regex: Pattern = re.compile('.*nk-iv-wg4-overview.*')
        date_li_elements: List[Tag] = soup.find_all("ul", {"class": regex})[0].find_all("li")
...
class ZippyshareParser(BaseParser):
    def get_download_link(self, page_link: str) -> str:
        pass

    def download_file(self, page_link: str, path: str) -> Dict[str, str]:
        pass
